...
Status: **WAVE 9 REVIEW PASS — CURRENT LOCAL INSTALL/UPGRADE PASS — WAVE 10 HOSTED CI PASS**. This packet records implementation, fresh root verification, five-lane adversarial PASS, the current vendored dependency-graph install proof, and hosted run `29789044550`: harness, macOS app/UI, and Windows TUI all passed. It does not authorize a public binary release or claim Developer ID signed/notarized production readiness.
...
The `LightningLoopUI` build-for-testing result and the historical 2/2 runner result are fixtures, not proof of the current live UI journey. The source install/launch smoke completed after Wave 9 PASS; the separate Accessibility-driven live UI evidence journey remains REWORK. The Xcode UI-test runner must never be installed as the product or exempted from Gatekeeper.
...
- Before live installation, the macOS installer holds an exclusive lock, verifies both staged artifacts, directly extracts only reviewed regular packed entries, creates only reviewed aliases, and independently hashes every required production cache archive against its exact reviewed lockfile SRI before bounded create-new extraction. It never trusts root `node_modules`; the archive-comparison pass itself produces manifest authority for exact dependency paths/modes/sizes/hashes and stable real-directory container identities, while symlink containers and `.bin` executable subtrees fail closed. It binds every packed root path/type/mode/size/hash plus canonical package identity/dependencies/bin/engines and dependency integrity/version/package/tree hashes, creates a unique rollback snapshot, and uses same-volume `renameatx_np(RENAME_EXCL)` transitions so recreated targets cannot be nested into or overwritten.
- This is a recoverable transaction, not one filesystem-atomic set. Isolated macOS fixtures compare the complete prior GUI, TUI, `llp`/`lloop`/`lightningloop`, and unrelated-alias state after injected mid-backup, mid-commit, installed-signature, and launch-smoke failures. The lock remains held through rollback/cleanup; rollback failures return nonzero. Old exact app processes must be gone before replacement, and launch proof binds one stable new PID to the installed executable. The post-PASS install completed this live proof at `~/Applications/LightningLoop.app` and `~/.local/bin`.
- Windows performs the analogous per-target locked, recreated-target-rejecting, lock-bound production install, post-commit dependency/container-manifest verification, package/shim backup, and byte-manifest rollback. Hosted run `29789044550` passed this complete Windows matrix with 135 platform-applicable dependencies.
...
| UI fixture and checklist language overclaimed live proof | The fixture remains visibly labeled `UI TEST FIXTURE` / `NOT CURRENT VERIFICATION` with false proof flags. Later proof moved `LL-009` compilation and `LL-025` source install/launch to LOCAL; only the separate Accessibility-driven `LL-010` live UI evidence journey remains REWORK | UI build-for-testing, completed source install/launch smoke, and current checklist scan; no fixture is treated as live Accessibility proof |
| Custom-provider and Windows wording overstated proof | Docs state user-trusted HTTPS hostname validation without native DNS pinning; Windows remains a source-install claim | documentation diff plus hosted Windows run `29789044550` PASS |

## Remaining release boundaries

GitHub source installation and hosted run `29789044550` completed the source-level merge proof. A public macOS binary release remains REWORK and blocked on a bundled runtime/license design, Developer ID identity, hardened-runtime entitlements, notarization/stapling, signed update feed, clean-machine install/update/rollback proof, and release operations. Windows proof is limited to the hosted source installer; no signed distribution claim, tag, or GitHub Release is part of this change.
...
| Windows partial backup could delete untouched shims and upgrades retained obsolete aliases | track only moved-old/installed-new paths, discover at most 32 safe prior aliases, restore exact byte manifests after mid-backup/commit/smoke failure, remove obsolete aliases only on success | committed and hosted `windows-2025` rollback/upgrade fixture PASS in run `29789044550` |
...
| Packed installers performed a later dependency resolution not bound to the reviewed lock, and macOS rollback could mask a failed restoration | both platforms directly extract the reviewed root archive, independently SRI-verify production cache archives, bind exact dependency trees/containers across the recoverable move, and propagate accumulated rollback failures | 136 local macOS and 135 hosted Windows dependency/container records; poisoned-cache rejection; macOS backup/commit/signature/smoke rollback; hosted Windows PASS |
...
| The documented mac installer was not executable; installed dependencies could be re-resolved outside the lock; partial rollback could be masked | executable installer; both platforms directly extract reviewed root/cache archives, revalidate lock-SRI package trees and real container identities after commit, and compare complete rollback state | 136 local macOS and 135 hosted Windows dependencies, poisoned-cache rejection, mac rollback fixtures, ShellCheck/YAML/package gates, hosted Windows PASS |

At the Wave 4 boundary, smoke remained deliberately deferred pending fresh review of the repaired diff and root proof; those later review/install gates are recorded below.
...
At the Wave 6 boundary, install smoke remained deliberately deferred pending fresh review; the later Wave 9 install and Wave 10 hosted results are recorded below.
...
| Packed root CLI/skills were not integrity-bound | a bounded tar parser validates the exact 46-file archive; runtime manifest schema 2 binds every packed root path/type/mode/size/hash and the archive hash, then reopens the installed tree plus platform-applicable lock/SRI-bound dependencies before any CLI execution | mutated `dist/cli/index.js`, symlink-container, `.bin`, post-extraction mutation, archive, dependency, and poisoned-cache rejection proofs |
| Windows arbitrary prefixes could cross volumes or alias one target through reparse paths; both installers had check/use target races | Windows resolves identity, rejects reparse/junction paths, stages/backups on the live volume, and uses same-volume .NET no-replace renames; macOS uses compiled `renameatx_np(RENAME_EXCL)` atomic directory transitions; both verify postconditions and retain exact rollback state | macOS race/rollback/lock/process fixtures and ShellCheck PASS; hosted Windows cross-volume/reparse/race matrix PASS in `29789044550` |
...
At the Wave 7 boundary, install smoke remained deliberately deferred pending fresh review; the later Wave 9 install and Wave 10 hosted results are recorded below.
