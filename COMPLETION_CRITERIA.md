...
- **Increment one-minute setup / skills (same PR):** First-run is four steps; doctor/help name Next. Default pack list/enable/disable is explicit. Settings cut to Setup / Skills / Harness. Linux gates: `check:harness` 0, `test:portable` 201/200/1 skip, `build:harness` 0. **No GUI row is DONE.**
- **Increment models/keys/engine (this run):** Shared CLI/TUI/serve key resolution now includes Firecrawl/Exa/Brave and the OS store on Linux; Discover/`--model` fail closed against the correct catalog. Linux gates: `check:harness` 0, `test:portable` 193/192/1 skip, `build:harness` 0, plus isolated CLI probes (catalog fail-closed, key status, search next-action, doctor). GitHub Actions run 33190079302: harness, windows-tui, macos-app green (`xcodebuild test` + UI `build-for-testing`). **No GUI row is DONE.** CC-A6 harness fail-closed is CLI-proven; live inference still pending. CC-A7 and CC-A11 remain HARNESS-DONE / GUI pending. LL-010/LL-011 stay REWORK. LL-013–017, LL-021, LL-022 stay MISSING.
- **Increment model pick on the installed app (same PR, 2026-08-28):** CLI `provider models` / `pick` / `add`, TUI `/models`, and GUI Load models + catalogued picker persist only listed IDs. Unknown IDs and missing host keys fail closed (`model_unavailable` / `key set NAME`). OpenRouter catalog stays credential-free (PR 17 Keychain silence). Fusion (CC-A9) untouched. Linux: `check:harness` 0, `test:portable` 213/212/1 skip. Isolated CLI proved list/add/pick-by-index plus both fail-closed paths. **CC-A4 / CC-A6 GUI remain not DONE** — this Linux VM cannot run xcodebuild; macos-app CI must stay green. No production row flipped.
