...
- **Increment models/keys/engine (this run):** Shared CLI/TUI/serve key resolution now includes Firecrawl/Exa/Brave and the OS store on Linux; Discover/`--model` fail closed against the correct catalog. Linux gates: `check:harness` 0, `test:portable` 193/192/1 skip, `build:harness` 0, plus isolated CLI probes (catalog fail-closed, key status, search next-action, doctor). **No GUI row is DONE.** CC-A6 harness fail-closed is CLI-proven; live inference still pending. CC-A7 and CC-A11 remain HARNESS-DONE / GUI pending. LL-010/LL-011 stay REWORK. LL-013–017, LL-021, LL-022 stay MISSING.
