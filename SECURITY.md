...
- Pi owns credentials for every named built-in inference provider. Research keys and an explicit native custom-provider credential are stored as separate generic passwords in macOS Keychain using `AfterFirstUnlockThisDeviceOnly` accessibility; LightningLoop does not read Pi credentials.
...
- Every named built-in provider preset is Pi-managed. A custom inference profile requires an explicit user-trusted, credential-free public HTTPS DNS hostname; literal IPs, localhost/local names, query strings, fragments, and cross-origin authentication redirects are rejected. Native custom-provider connection testing does not DNS-pin its connection, so hostname validation is not a complete DNS-rebinding/SSRF defense; loop execution and Gold remain blocked unless the shared harness is available.
