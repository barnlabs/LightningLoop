...
Clone the canonical BarnLabs repository, then use the checked-in executable installer. It verifies the complete harness, creates a universal Release build, ad-hoc signs that local build, and stages a locked, recoverable GUI/TUI transaction in `~/Applications` and `~/.local/bin`. This is not one filesystem-atomic set: an exclusive install lock prevents cooperating concurrent installers, each directory transition uses same-volume `renameatx_np(RENAME_EXCL)`, and a rollback snapshot restores the prior set on a failed commit. The TUI's production dependency tree is rebuilt with `npm ci --omit=dev --ignore-scripts` from the reviewed `package-lock.json`; a bounded archive parser and runtime manifest bind all 44 packed paths/types/modes/sizes/hashes, archive/package/bin/engine metadata, and 148 dependency integrity/version/tree records, then verify the installed tree again before any CLI execution. The live install and normal Finder-launch smoke are intentionally deferred until the current adversarial repair packet receives fresh PASS; do not treat the installer tests as that live proof. Shell-only Node managers such as nvm, fnm, asdf, and Volta are not sufficient for normal Finder launch unless their supported Node binary is linked or installed at `~/.local/node/bin/node`.
...
Use PowerShell with Node.js 22.19+ and Git for Windows installed. The checked-in installer verifies the portable contracts, packs the allowlisted CLI, discards the global install's dependency resolution, and rebuilds the production tree from the reviewed lock with lifecycle scripts disabled and offline. The same complete packed-root/package/dependency manifest is checked after commit. A resolved-prefix exclusive file lease is held through rollback and cleanup; staging and backup stay on that live volume; reparse/junction/cross-volume prefixes and recreated package/shim targets fail closed through same-volume .NET no-replace renames. The committed `windows-2025` workflow has not yet produced its first hosted-run evidence, so neither local script review nor this documentation is a Windows smoke or release claim.
